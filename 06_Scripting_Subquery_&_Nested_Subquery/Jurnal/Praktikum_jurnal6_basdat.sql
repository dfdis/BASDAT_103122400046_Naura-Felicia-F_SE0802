...
FROM 
    MEMBER m
JOIN 
    TRANSAKSI tr ON m.ID_MEMBER = tr.ID_MEMBER
JOIN 
    JADWAL j ON tr.ID_JADWAL = j.ID_JADWAL
JOIN 
    FILM f ON j.ID_FILM = f.ID_FILM
JOIN 
    THEATER t ON j.ID_THEATER = t.ID_THEATER
WHERE 
...
FROM 
    FILM f
JOIN 
    JADWAL j ON f.ID_FILM = j.ID_FILM
JOIN 
    THEATER t ON j.ID_THEATER = t.ID_THEATER
WHERE 
    t.HARGA = (SELECT MIN(HARGA) FROM THEATER)
    OR 
    t.HARGA = (SELECT MAX(HARGA) FROM THEATER)
ORDER BY 
    t.HARGA ASC;
